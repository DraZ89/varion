...
function getMatchTimestamp(match, sport) {
  if (sport === "football") {
    // Foot : date "2026-04-30", kickoff "20:00"
    const date = match.date || "";
    const time = match.kickoff || "00:00";
    return `${date}T${time}`;
  } else {
    // Tennis : date "2026-04-30", time "14:30"
    const date = match.date || "";
...
window.HomePage = function HomePage({ onMatchClick }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 fade-in">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        {[
          { id: "all", label: "TOUS" },
          { id: "football", label: `FOOT (${foot.length})` },
          ...
          return (
            <div key={`${m._sport}-${m.id}`} style={{ animationDelay: `${i * 30}ms` }} className="fade-in flex justify-center">
              <window.CompactCard
                match={m}
                sport={m._sport}
                onClick={m._sport === "football" ? () => onMatchClick(m.id) : null}
              />
            </div>
          );
        })}
